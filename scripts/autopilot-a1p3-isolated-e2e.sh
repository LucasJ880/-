#!/usr/bin/env bash
# Isolated local Postgres E2E for Autopilot A1-P3 Observe Dashboard.
...
PGDATA="${TMPDIR:-/tmp}/qingyan-a1p3-pgdata"
PGLOG="${TMPDIR:-/tmp}/qingyan-a1p3-pg.log"
MODE=""
...
start_local_pg17() {
  if [[ ! -x "$PG17_BIN/postgres" ]]; then
    echo "neither docker nor postgresql@17 is available for isolated A1-P3 E2E" >&2
    exit 1
  fi
...
echo "running A1-P3 isolated E2E..."
