#!/usr/bin/env bash
# Isolated local Postgres E2E for Autopilot A1-P2 Human Signals.
...
PGDATA="${TMPDIR:-/tmp}/qingyan-a1p2-pgdata"
PGLOG="${TMPDIR:-/tmp}/qingyan-a1p2-pg.log"
MODE=""
...
start_local_pg17() {
  if [[ ! -x "$PG17_BIN/postgres" ]]; then
    echo "neither docker nor postgresql@17 is available for isolated A1-P2 E2E" >&2
    exit 1
  fi
...
echo "running A1-P2 isolated E2E..."
