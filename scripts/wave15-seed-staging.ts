#!/usr/bin/env tsx
/**
 * Wave1.5 Staging 合成测试数据（仅非生产库）
 *
 * 要求：
 *   QINGYAN_RUNTIME_ENV=staging
 *   DATABASE_URL 不得为 ep-super-field*
 *   WAVE15_SEED_PASSWORD 提供统一测试密码（不写入仓库）
 *
...
function mustStaging() {
  const runtime = (process.env.QINGYAN_RUNTIME_ENV || "").toLowerCase();
  if (runtime !== "staging") {
    throw new Error("拒绝：需要 QINGYAN_RUNTIME_ENV=staging");
  }
  const url = process.env.DATABASE_URL || "";
  const prefix = extractDbEndpointPrefix(url);
  if (!prefix || isProductionDatabaseUrl(url)) {
    throw new Error("拒绝：DATABASE_URL 为生产 endpoint 或无法解析");
  }
  console.log(`[wave15-seed] target endpoint_prefix=${prefix}`);
}
...
async function main() {
  ...
  const admin = await db.user.upsert({
    ...
    update: { passwordHash: hash, status: "active", role: "admin", name: "WAVE15 TEST Admin", orgAccessMode: "MULTI_ORG", canSelfSwitchOrg: true },
    create: {
  ...
  const member = await db.user.upsert({
    ...
    update: { passwordHash: hash, status: "active", role: "user", name: "WAVE15 TEST Member" },
    create: {
  ...
  const outsider = await db.user.upsert({
    ...
    update: { passwordHash: hash, status: "active", role: "user", name: "WAVE15 TEST Outsider" },
    create: {
  ...
  const orgA = await ensureOrg("wave15-test-org-a", "WAVE15 TEST Org A", admin.id);
  const orgB = await ensureOrg("wave15-test-org-b", "WAVE15 TEST Org B", admin.id);
  ...
  async function ensureCustomer(orgId: string, createdById: string, name: string) {
  ...
  const custA = await ensureCustomer(orgA.id, admin.id, "WAVE15 TEST Customer A");
  const custB = await ensureCustomer(orgB.id, outsider.id, "WAVE15 TEST Customer B");

  async function ensureQuote(orgId: string, customerId: string, createdById: string) {
    const found = await db.salesQuote.findFirst({ where: { orgId, customerId } });
    if (found) return found;
  ...
  async function ensureProject(orgId: string, ownerId: string, name: string, code: string) {
  ...
  const projA = await ensureProject(orgA.id, admin.id, "WAVE15 TEST Project A", "WAVE15-PA");
  const projB = await ensureProject(orgB.id, outsider.id, "WAVE15 TEST Project B", "WAVE15-PB");

  const pending = await db.pendingAction.findFirst({
    where: { orgId: orgA.id, type: "grader.internal_note", status: "pending" },
  });
  const pendingId =
    pending?.id ||
    (
  ...
  const summary = {
    staging_endpoint_prefix: extractDbEndpointPrefix(process.env.DATABASE_URL || ""),
    org_a_id: orgA.id,
    org_b_id: orgB.id,
    ...
    password_sha256: createHash("sha256").update(password).digest("hex").slice(0, 12) + "…",
  };
