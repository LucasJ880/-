...
run_test "Mention Gateway M2-B Binding Policy（纯逻辑）" "NODE_ENV=test npx tsx src/lib/mention-gateway/__tests__/m2b-binding-policy.test.ts"
run_test "Mention Gateway M2-B Binding DB（隔离库执行，否则跳过）" "NODE_ENV=test npx tsx src/lib/mention-gateway/__tests__/m2b-binding-db.isolated.test.ts"
run_test "Mention Gateway M2-B DB 全链路 E2E（隔离库执行，否则跳过）" "NODE_ENV=test npx tsx src/lib/mention-gateway/__tests__/m2b-db-full-e2e.isolated.test.ts"
