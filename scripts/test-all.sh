...
run_test "Revenue Spine DB 端到端(isolated)" "npx tsx src/lib/revenue-spine/__tests__/revenue-spine-db.isolated.test.ts"
...
run_test "Mention Gateway M1 Final Review（B1 幂等边界 / B2 终态顺序）" "NODE_ENV=test npx tsx src/lib/mention-gateway/__tests__/m1-final-review.test.ts"
run_test "Mention Gateway M2-C Context Tool Policy + Customer Scope Guard" "NODE_ENV=test npx tsx src/lib/mention-gateway/__tests__/m2c-tool-policy.test.ts"
run_test "Mention Gateway M2-A Identity Policy（纯逻辑）" "NODE_ENV=test npx tsx src/lib/mention-gateway/__tests__/m2a-identity-policy.test.ts"
run_test "Mention Gateway M2-A Identity DB（隔离库执行，否则跳过）" "NODE_ENV=test npx tsx src/lib/mention-gateway/__tests__/m2a-identity-db.isolated.test.ts"
run_test "Mention Gateway M2-A DB Identity E2E（隔离库执行，否则跳过）" "NODE_ENV=test npx tsx src/lib/mention-gateway/__tests__/m2a-db-identity-e2e.isolated.test.ts"
...
run_test "乱码时代生成文档列表作废（jsPDF/回落件隐藏）" "npx tsx src/lib/projects/generate/__tests__/legacy-doc-filter.test.ts"
...
run_test "Supplier Intel S1 审计脊柱 DB（隔离库执行，否则跳过）" "NODE_ENV=test npx tsx src/lib/supplier-intel/__tests__/supplier-intel-db.isolated.test.ts"
run_test "Supplier Intel S2 SearchBrief 生成器（确定性+LLM 两跳降级）" "npx tsx src/lib/supplier-intel/__tests__/search-brief.test.ts"
run_test "Supplier Intel S2 Provider 策略门（T7 fail-closed）" "npx tsx src/lib/supplier-intel/__tests__/providers-policy.test.ts"
...
run_test "Supplier Intel S2-REM BL-2 扫描完整性贯穿全分支（纯核表驱动）" "npx tsx src/lib/supplier-intel/__tests__/identity-scan.test.ts"
run_test "Supplier Intel S2-REM BL-4A egress 查询过滤（真实计划→绑定→discover，零 DB/零 key）" "npx tsx src/lib/supplier-intel/__tests__/egress-plan.test.ts"
run_test "Supplier Intel S2-REM BL-4B Run 收口决策（§44 语义）" "npx tsx src/lib/supplier-intel/__tests__/run-finalization.test.ts"
run_test "Supplier Intel S2-REM 整改回归 DB（BL-2 快照/BL-3 边界行为/BL-4 收口持久化；隔离库执行，否则跳过）" "NODE_ENV=test npx tsx src/lib/supplier-intel/__tests__/supplier-intel-s2rem-db.isolated.test.ts"
...
run_test "Autopilot A1-P2 Human Signals" "npx tsx src/lib/autopilot/__tests__/human-signals.test.ts"
...
run_test "Autopilot A2-P2.2 Semantic Judge Input" "npx tsx src/lib/autopilot/__tests__/a2p2-semantic-judge-input.test.ts"
...
run_test "Autopilot A2-P2.4 Exception Envelope" "npx tsx src/lib/autopilot/__tests__/a2p2-exception-envelope.test.ts"
run_test "Autopilot A2-P2.4 A2 Closure Matrix" "npx tsx src/lib/autopilot/__tests__/a2p2-a2-closure-matrix.test.ts"
...
run_test "GPT6 Astra Flags" "npx tsx src/lib/ai/model-policy/__tests__/flags.test.ts"
run_test "GPT6 Astra Resolve" "npx tsx src/lib/ai/model-policy/__tests__/resolve.test.ts"
run_test "GPT6 Astra Reasoning" "npx tsx src/lib/ai/model-policy/__tests__/reasoning.test.ts"
run_test "GPT6 Astra Compat" "npx tsx src/lib/ai/model-policy/__tests__/compat.test.ts"
run_test "GPT6 Astra Retry" "npx tsx src/lib/ai/model-policy/__tests__/retry.test.ts"
run_test "GPT6 Astra Guardrails" "npx tsx src/lib/ai/model-policy/__tests__/guardrails.test.ts"
...
run_test "T5 Seg2 canonical V2 持久化脊柱" "npx tsx src/lib/tender-workforce/__tests__/t5-seg2-v2-spine.test.ts"
...
run_test "情报自动流 七槽位自动化" "npx tsx src/lib/tender-intel/__tests__/intel-auto-flow.test.ts"
run_test "CJK-PDF 中文安全文档生成" "npx tsx src/lib/pdf/__tests__/cjk-pdf.test.ts"
...
run_test "合规矩阵可用性批次" "npx tsx src/lib/tender-auto-analysis/__tests__/bid-fit-usability.test.ts"
...
run_test "Quote Engine 计算" "npx tsx src/lib/quote-engine/__tests__/quote-calc.test.ts"
run_test "Quote Engine 契约与安全" "npx tsx src/lib/quote-engine/__tests__/quote-engine-contract.test.ts"
...
run_test "分析师备忘录 v2（全文分块/接地/数字回核/两跳市场/续跑协议）" "npx tsx src/lib/tender-analyst-memo/v2/__tests__/memo-v2.test.ts"
