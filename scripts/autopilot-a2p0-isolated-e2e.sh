#!/usr/bin/env bash
# Isolated local Postgres E2E for Autopilot A2-P0 Deterministic Evaluate.
...
PGDATA="${TMPDIR:-/tmp}/qingyan-a2p0-pgdata"
PGLOG="${TMPDIR:-/tmp}/qingyan-a2p0-pg.log"
MODE=""
...
start_local_pg17() {
  if [[ ! -x "$PG17_BIN/postgres" ]]; then
    echo "neither docker nor postgresql@17 is available for isolated A2-P0 E2E" >&2
    exit 1
  fi
...
echo "running A2-P0 isolated E2E..."
