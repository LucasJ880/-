...
interface DashboardData {
  ...
  summary: { marketPresence: number | null; growthExecution: number; effectiveLeads: number; revenue: number; currency: string; runningExperiments: number; pendingContent: number; highPriorityIssues: number; spend: number };
  ...
  plan: { id: string; name: string; items: Array<{ id: string; title: string; dueDate: string; priority: string; status: string }> } | null;
}
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    {data && <>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <section className="rounded-xl border border-border bg-card-bg p-4"><div className="flex items-center justify-between"><h2 className="font-semibold">30 天推广计划</h2><button type="button" onClick={generatePlan} className="text-sm text-accent">生成新计划</button></div>{data.plan ? <div className="mt-3"><div className="text-sm font-medium">{data.plan.name}</div><div className="mt-2 space-y-2">{data.plan.items.map((item) => <div key={item.id} className="flex justify-between gap-3 text-sm"><span>{item.title}</span><span className="shrink-0 text-xs text-muted">{item.dueDate.slice(0,10)}</span></div>)}</div></div> : <p className="mt-3 text-sm text-muted">体检后可按问题严重度生成 30 天计划。</p>}</section>
      </div>
    </>}
  </div>;
}
