...
import { Copy, Check, Link as LinkIcon, RotateCcw, X, Trash2, Eye } from "lucide-react";
...
export default function ShareDialog(props: ShareDialogProps) {
  ...
  const [busy, setBusy] = useState<null | "create" | "revoke" | "renew">(null);
  ...
  const remaining = diffDays(session.shareExpiresAt);

  const expired = useMemo(() => {
    if (!session.shareExpiresAt) return false;
    return new Date(session.shareExpiresAt).getTime() <= Date.now();
  ...
  const renew = create; // 复用同一接口（覆盖 token + 重置过期时间）
