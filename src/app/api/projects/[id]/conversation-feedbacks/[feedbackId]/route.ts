...
export async function GET(request: NextRequest, ctx: Ctx) {
  ...
  const feedback = await db.conversationFeedback.findFirst({
...
export async function PATCH(request: NextRequest, ctx: Ctx) {
  ...
  const updated = await db.conversationFeedback.update({
    where: { id: feedbackId },
    data: updates,
    include: { tags: { include: { tag: true } } },
  });
  ...
  await logAudit({
    ...
    orgId: project.orgId,
    projectId,
    action: AUDIT_ACTIONS.UPDATE_CONVERSATION_FEEDBACK,
