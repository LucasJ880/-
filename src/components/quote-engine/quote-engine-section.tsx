  ...
  return (
    <div className="rounded-xl border border-border bg-card-bg p-4" data-testid="quote-engine-section">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {data.capabilities.canEdit ? <div className="flex flex-wrap gap-2 text-[11px]"><button type="button" disabled={busy} onClick={() => void create("PROJECT_SUPPLY_INSTALL")} className="rounded border border-border bg-accent/10 px-2 py-1">+ Supply + Install</button><button type="button" disabled={busy} onClick={() => void create("STANDING_OFFER")} className="rounded border border-border bg-accent/10 px-2 py-1">+ Standing Offer</button><button type="button" disabled={busy} onClick={() => void create("SUPPLY_ONLY")} className="rounded border border-border bg-accent/10 px-2 py-1">+ Supply Only（供货不安装）</button>{process.env.NODE_ENV !== "production" ? <><button type="button" disabled={busy} onClick={() => void create("PROJECT_SUPPLY_INSTALL", "A")} className="rounded border border-dashed border-border px-2 py-1 text-muted">Demo A</button><button type="button" disabled={busy} onClick={() => void create("STANDING_OFFER", "B")} className="rounded border border-dashed border-border px-2 py-1 text-muted">Demo B</button></> : null}</div> : null}
      </div>
