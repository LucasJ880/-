...
export function AiServiceConfigHint({ variant = "full" }: { variant?: Variant }) {
  if (variant === "compact") {
    return (
      <div className="rounded-[var(--radius-md)] border border-sky-500/25 bg-sky-500/10 px-4 py-3 text-sm text-foreground">
  ...
  return (
    <div className="mx-auto flex h-full max-w-xl flex-col items-center justify-center gap-4 px-4 text-center">
      <div className="flex h-16 w-16 items-center justify-center rounded-[var(--radius-xl)] bg-gradient-to-br from-sky-500 to-indigo-600 text-white shadow-card">
        <Bot size={32} />
      </div>
