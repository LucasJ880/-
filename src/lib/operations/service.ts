 ...
 *
 * 流程：AI 生成每账号差异化文案 → 规则检查 →
 * - block：滞留 status=blocked，不派发
 * - review / 抽检命中：滞留 status=review，人工通过后派发
...
export async function fanoutAndDispatch(input: FanoutInput): Promise<FanoutResult> {
  ...
  for (const [i, account] of accounts.entries()) {
    ...
    const sampled = i % SAMPLE_EVERY_N === 0;

    const initialStatus =
      rule.verdict === "block" ? "blocked" : rule.verdict === "review" || sampled ? "review" : "draft";
    ...
    const job = await db.publishJob.create({
      data: {
        ...
        errorMessage: rule.reasons.length ? rule.reasons.join("；") : null,
      },
    });
    createdJobs += 1;
  ...
  return { createdJobs, queued, held, blocked, failed, variantFallback: usedFallback, errors };
}
