...
ok(svc.includes("metadata: metadata as Prisma.InputJsonValue") && svc.includes("sourceDocumentId") && svc.includes("originalDescription") && svc.includes("originalAmount"), "QO-09: Apply 后成本行 metadata 带 importId/sourceDocumentId/原始描述/原始金额 provenance");
...
ok(/BID_ELIGIBLE = \["approved", "awarded"\]/.test(tb) && tb.includes("NOT_APPROVED") && tb.includes("QUOTE_REVISION_PENDING") && tb.includes("latestEligibleInLineage") && tb.includes("ourBidPrice: selling") && tb.includes("QUOTE_SELECTED_AS_TENDER_BID"), "QO-19: 仅 approved/awarded 可成为我方报价；superseded 跟随修订或 QUOTE_REVISION_PENDING；写穿 ourBidPrice；审计");
const service = code("src/lib/quote-engine/service.ts");
ok((service.match(/syncTenderBidPointer\(/g) ?? []).length >= 2, "QO-20: transitionQuote / reviseQuote 后同步我方报价指针");
