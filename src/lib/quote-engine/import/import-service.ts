...
import { getQuote, QuoteEngineError, updateEngineQuote, type QuoteRecord } from "../service";
...
export async function createImportFromUpload(input: {
  ...
  const defaultCurrency = (input.defaultCurrency ?? quote.currency).toUpperCase();
  let record = await db.quoteCostImport.create({
    data: {
      ...
      metadataJson: { defaultCurrency, reimportOf: dup?.id ?? null, mime: input.file.mime, size: input.file.size } as Prisma.InputJsonValue,
  ...
  try {
    ...
    if (sourceType === "PDF") {
      ...
      extraction = extractRowsFromPdfPages(pdf.pages.map((p) => ({ pageNumber: p.pageNumber, contentText: p.contentText })), { defaultCurrency });
    } else {
      extraction = extractRowsFromWorkbook(input.file.buffer, { defaultCurrency, sourceType });
    }
    ...
    record = await db.quoteCostImport.update({
      ...
      data: {
        ...
        metadataJson: { ...(record.metadataJson as Record<string, unknown>), detectedCurrency: extraction.detectedCurrency, notes: extraction.notes, sheets: extraction.sheets, pages: extraction.pages, aiUpdated, rowCount: extraction.rows.length } as Prisma.InputJsonValue,
      },
    });
...
export async function updateImportReview(input: { importId: string; quoteId: string; projectId: string; orgId: string; userId: string; patch: unknown }): Promise<ImportRecord> {
  ...
  const updated = await db.quoteCostImport.update({
    ...
    data: {
      ...
      ...(patch.defaultCurrency !== undefined ? { metadataJson: { ...meta, defaultCurrency: patch.defaultCurrency } as Prisma.InputJsonValue } : {}),
    },
  });
...
function existingLinePayload(l: QuoteRecord["costLines"][number]): CostLinePayload {
  const n = (v: Prisma.Decimal | null) => (v == null ? null : Number(v));
  return { id: l.id, sortOrder: l.sortOrder, category: l.category, subcategory: l.subcategory, description: l.description, quantity: n(l.quantity), unit: l.unit, unitCost: n(l.unitCost), sourceCurrency: l.sourceCurrency, fxRate: n(l.fxRate), fxRateSource: l.fxRateSource, calculationType: l.calculationType as CostLinePayload["calculationType"], calculationBase: l.calculationBase, rate: n(l.rate), duration: n(l.duration), supplierId: l.supplierId, supplierName: l.supplierName, source: l.source, notes: l.notes, included: l.included };
}
...
export function rowToCostLinePayload(row: ImportRow, ctx: { importId: string; sortOrder: number; supplierName: string | null; quoteCurrency: string }): CostLinePayload {
  ...
  const currency = (row.sourceCurrency ?? ctx.quoteCurrency).toUpperCase();
  ...
  return {
    ...
    // 外币必须人工填汇率（与 B1 fail-closed 一致：引擎校验 FX_REQUIRED 前不能定价）
    fxRate: currency === ctx.quoteCurrency.toUpperCase() ? null : null,
    fxRateSource: null,
...
export async function applyImport(input: { importId: string; quoteId: string; projectId: string; orgId: string; userId: string }): Promise<{ record: ImportRecord; quote: QuoteRecord; lineIds: string[] }> {
  const record = await getImport(input);
  if (record.status !== "CONFIRMED") throw new QuoteEngineError("IMPORT_INVALID_STATE", `状态 ${record.status} 不能应用（需先确认）`, 409);
  const quote = await getQuote(input.quoteId, input.projectId);
  assertQuoteImportable(quote);
  const rows = rowsOf(record).filter((r) => r.include);
  const issues = validateRowsForConfirm(rows);
  if (issues.length > 0) throw new QuoteEngineError("IMPORT_ROWS_INVALID", "行校验未通过", 422, { issues });
  const maxSort = quote.costLines.reduce((m, l) => Math.max(m, l.sortOrder), 0);
  const existing = quote.costLines.map(existingLinePayload);
  const fresh = rows.map((r, i) => rowToCostLinePayload(r, { importId: record.id, sortOrder: maxSort + (i + 1) * 10, supplierName: record.supplierName, quoteCurrency: quote.currency }));
  const updated = await updateEngineQuote({ quoteId: input.quoteId, projectId: input.projectId, userId: input.userId, lines: [...existing, ...fresh] });
  const newLines = updated.costLines.filter((l) => l.source === `import:${record.id}` && !quote.costLines.some((x) => x.id === l.id));
  // provenance → QuoteCostLine.metadata（按描述+排序配对；同批次内 sortOrder 唯一）
  const bySort = new Map(fresh.map((f, i) => [f.sortOrder, rows[i]!]));
  await db.$transaction(
    newLines.map((l) => {
      const row = bySort.get(l.sortOrder);
      const metadata = row
        ? { importId: record.id, sourceDocumentId: record.sourceDocumentId, sourceFilename: record.sourceFilename, rowId: row.rowId, sheet: row.evidence.sheet, row: row.evidence.row, cell: row.evidence.cell, pageNumber: row.evidence.pageNumber, unitLabel: row.evidence.unitLabel, snippet: row.evidence.snippet, originalDescription: row.sourceDescription, originalAmount: row.sourceAmount, rawAmountText: row.rawAmountText, sourceCurrency: row.sourceCurrency, supplierName: record.supplierName, quoteDate: record.quoteDate ? record.quoteDate.toISOString().slice(0, 10) : null, extractionVersion: record.extractionVersion, confidence: row.confidence, aiSuggested: row.aiSuggested, userEdited: row.userEdited }
        : { importId: record.id, sourceDocumentId: record.sourceDocumentId };
      return db.quoteCostLine.update({ where: { id: l.id }, data: { metadata: metadata as Prisma.InputJsonValue } });
    }),
  );
  const lineIds = newLines.map((l) => l.id);
  const done = await db.quoteCostImport.update({ where: { id: record.id }, data: { status: "APPLIED", appliedAt: new Date(), appliedById: input.userId, appliedJson: { lineIds, count: lineIds.length, appliedAt: new Date().toISOString() } as Prisma.InputJsonValue } });
  await audit({ userId: input.userId, orgId: input.orgId, projectId: input.projectId, action: IMPORT_AUDIT_ACTIONS.QUOTE_IMPORT_APPLIED, importId: record.id, after: { lineIds, count: lineIds.length, quoteVersion: quote.version } });
  return { record: done, quote: await getQuote(input.quoteId, input.projectId), lineIds };
}
...
export function serializeImport(r: ImportRecord, opts: { withRows: boolean }) {
  ...
  return {
    ...
    notes: (meta.notes as string[] | undefined) ?? [], detectedCurrency: (meta.detectedCurrency as string | null | undefined) ?? null, defaultCurrency: (meta.defaultCurrency as string | undefined) ?? null, reimportOf: (meta.reimportOf as string | null | undefined) ?? null,
