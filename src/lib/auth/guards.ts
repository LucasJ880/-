 ...
 *
 * @example
 * const auth = await requireOrgPermission(request, orgId, "org:member:invite");
 * if (auth instanceof NextResponse) return auth;
 */
 ...
 *
 * @example
 * const auth = await requireProjectPermission(request, orgId, projectId, "prompt:publish");
 * if (auth instanceof NextResponse) return auth;
 */
export async function requireProjectPermission(
  ...
  if (!hasProjectPermission(projectMembership.role, permission)) {
    return NextResponse.json(
      { error: `缺少权限: ${permission}` },
      { status: 403 }
    );
