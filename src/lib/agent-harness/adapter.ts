...
import { runAgent } from "@/lib/agent-core/engine";
...
export async function runWithHarness(
  ...
  // 服务端 allowlist：空数组 = 无工具
  const allowed = [...new Set(input.allowedTools)];

  const toolFields = scopeToToolContextFields(scope);

  try {
    // 预检：禁止在消息外再信任客户端 org（messages 内容视为不可信数据，不当系统命令）
    for (const name of allowed) {
      // 占位：allowlist 本身由调用方计算；此处仅记录空名
      if (!name.trim()) {
    ...
    // 从工具结果中收集 PendingAction 提案标记（不执行）
