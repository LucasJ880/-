...
export async function getMarketingDashboard(orgId: string) {
  ...
  const [profile, findings, campaigns, runningExperiments, pendingContent, publications, metrics, attributions, plans] = await Promise.all([
    ...
    db.marketingPlan.findMany({ where: { orgId, status: { in: ["draft", "active"] } }, include: { items: { where: { status: { notIn: ["done", "completed", "canceled"] } }, orderBy: { dueDate: "asc" }, take: 8 } }, orderBy: { createdAt: "desc" }, take: 1 }),
  ]);
