...
>
> 青砚侧实现:`src/lib/operations/aivora.ts`(适配器)+
> `syncAivoraVideosForOrg`(`src/lib/operations/service.ts`)+
> `/api/cron/aivora-sync`(每 15 分钟)。
...
## 附:青砚侧实现要点(2026-08-03)

- **游标不落新表**:since 从已入库 `VideoAsset.metadataJson.completedAt` 派生
  `max()`,起点回退 1s 防同刻边界(接口幂等,重复无害)。cron / 手动触发 / 半途
  失败共用同一游标语义,自愈续接。
- **配方维度存 `metadataJson`**:`completedAt` / `recipeId` / `hookType` /
  `templateId` / `aspectRatio` / `brandPlacement`,枚举按 Aivora 原文存储。
  **null 保持 null**(= 未知);后续做配方赛马报表时必须 `WHERE ... IS NOT NULL`
  排除,不得归入默认桶。
- **更新不碰工作流字段**:重复拉到同 id 只刷新 Aivora 侧字段,`status` /
  `blockReason` / `orgId` 等青砚工作流字段不动;可空字段为 null 时不覆盖已有值。
- **观测**:每轮把 `pages` / `skippedUnbranded` / `since` / `nextSince` 写入
  AutomationRun.metadataJson;401/5xx 抛错记为 failed run,自动化面板可见。
- 环境变量:`AIVORA_API_URL` / `AIVORA_API_KEY` / `AIVORA_ORG_ID`(见
  `.env.example`);只读冒烟脚本:`npx tsx scripts/aivora-smoke-readonly.ts`。
