...
QO_TESTS                  = import 49/49 · customer 25/25 · contract 53/53 · performance 20/20 · Phase1 34/34+28/28 · guards 63/63+27/27 · golden DB E2E 69/69 · test-all 302/303（唯一失败为 main 既有）
...
## Final Review 2 Fix（2026-08-22，B2-CANCEL）

| 阻断 | 修复 | 证据 |
|---|---|---|
| 非权威态（被选报价 cancelled 且无替代 / superseded 待批）仍留着 `ourBidPrice` 与房间 `pricingInputs.ourPrice*`，legacy 读者会把已取消/已取代的报价当成当前 Our Bid | `clearBidMirrorsTx`（`bidQuoteId`/`ourBidPrice` = null，**`Project.currency` 不动**——它同时是项目基准币种；房间 `ourPriceCad/ourPrice/ourPriceCurrency/ourPriceSource` = null，`ourPriceStatus=NONE`）与 `writePendingMirrorsTx`（QUOTE_REVISION_PENDING：`bidQuoteId` 保留为可追溯的 superseded 来源，价格镜像全清，`ourPriceStatus=QUOTE_REVISION_PENDING` + `ourPriceSupersededSource`）；均由 `syncTenderBidPointerTx` 在 cancel / revise 的**同一事务**内调用（action `cleared` / `pending_cleared`，审计 `tender_bid_mirrors_cleared`）；AUTHORITATIVE 写入标 `ourPriceStatus=AUTHORITATIVE` 并清 superseded 痕迹。`resolveTenderBid.mirrorStale`：AUTHORITATIVE = 镜像价≠权威价；NONE/PENDING = 引擎已在用却仍宣传价格 | E2E B2-03b（PENDING 镜像语义逐字段）· B2-07a（取消无替代 → NONE + 全清，currency/竞对价不动 + 审计）· B2-07b（手动选新报价 → 原子恢复）· B2-07c（取消非被选 → 不动）· B2-07d（清空后批准唯一有效报价 → approve 事务内自动恢复）；结构守卫 B2-S7…S11；golden E2E **69/69**；quote-ops-contract **53/53**；迁移文件/schema 自审阅 head 零 diff（sha256 `ea5a74fb…`）；全量 test-all **302/303**（唯一失败仍是 main 既有 `Autopilot A2-P0 Isolated E2E`）；GitHub CI + Vercel – qingyan-staging **双绿 @ e45ba4d5**；隔离分支 `e2e-qops-fixgate2` 已删（残留 e2e-* = 0） |
