...
async function main() {
  ...
  // 先看队列里有什么（用户可能已点重新分析；没有就自动补一次 reanalyze）
  const PID = "cmsr0t06u0001l7044n50zuz5";
  const pending = await db.tenderAnalysisRun.findFirst({
  ...
  if (!pending) {
    ...
    console.log("PENDING_RUN " + pending.id);
  }
  ...
  const r = await db.$queryRaw<Array<Record<string, unknown>>>`
    SELECT "id","status",("summaryJson"->'analystSynthesis'->>'version') AS syn,
      ("summaryJson"->'brief'->>'recommendation') AS brief_rec
    FROM "TenderAnalysisRun" WHERE "projectId"=${PID} ORDER BY "createdAt" DESC LIMIT 1`;
  console.log("FINAL_RUN " + JSON.stringify(r[0]).slice(0, 350));
}
